...
class TaskController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($product_quantity < $request->target_quantity) {
            ...
            return redirect()->back()->with('error-message', 'This employee has task starting date, check it or wait for the 7 days');
        } elseif ($products  && $data->employee_id == $request->employee_id && $data->start_date != $request->start_date && $request->start_date < Carbon::create($data->start_date)->addDays(7)) {
            return redirect()->back()->with('error-message', 'This employee has task starting date, check it or wait for the next 7 days');
        }
        // dd($data);
        else {
            Task::create([
                ...
                'total_price' => $total_price,
                'start_date' => $request->start_date,
                'end_date' => Carbon::create($request->start_date)->addDays(7),
            ]);
